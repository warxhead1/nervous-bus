...
Three inputs, one run:
  1. `beads_global.all_issues` / `beads_global.all_dependencies` -- federated
     SQL views (adapters/board/create_view.sql, GENERATED by viewgen.py from
     the live per-DB column shape) UNIONing the `issues` / `dependencies`
     tables of the 10 project DBs (beads_global itself excluded -- it is the
     federation/routing DB, not a project with its own work queue). Both
     views are preflighted on every run (preflight_board_views) and a
     missing/mis-shaped/arm-dropped view aborts the run rather than emitting a
     board that silently under-reports blockers. Queried over the dolt
     server's MySQL-protocol port
...
try:
    import viewgen
except ImportError:  # pragma: no cover -- unusual invocation (python -m, embedding)
...
# main()'s exit code when a required source is missing/mis-shaped. Distinct
# from 1 so a systemd timer / caller can tell "the board sources are broken,
# go regenerate the views" from an ordinary crash.
...
class BoardSourceError(RuntimeError):
    """A required board source is missing or mis-shaped.

    Raised instead of returning a partial board. The federated views are the
    one input whose degradation is INVISIBLE in the output: if
    `all_dependencies` silently loses an arm, every bead in that project loses
    its `blocked_by` list and lands in the `ready` lane -- a board that looks
    healthier than reality and dispatches workers onto blocked work. So a
    missing/mis-shaped view is a hard failure, never a weak success.
    """


def preflight_board_views(conn, db: str = DOLT_DB) -> dict:
    """Fail closed unless both federated views exist, expose the columns this
    module reads, and still cover every roster DB.

    Three distinct failure modes, each of which previously looked like a
    healthy run:

      1. View absent / unqueryable. Happened for real on 2026-09-06: bd >= 1.2
         migrated deer_flow's `dependencies` to the split target columns while
         `all_dependencies` still selected `depends_on_id`, and dolt rejected
         the ENTIRE view rather than the one stale arm.
      2. View present but mis-shaped -- e.g. an older `all_dependencies`
         without `depends_on_kind`, which would make the blocked_by query
         below fail mid-run rather than up front with a remediation hint.
      3. View present, queryable, correctly shaped -- but regenerated with an
         arm DROPPED to make it apply. This is the dangerous one: counts look
         plausible and nothing errors. Detected by requiring every roster DB
         to appear in the view's own definition (SHOW CREATE VIEW), which is
         checked against the DEFINITION rather than against DISTINCT project
         values so a project that legitimately has zero issues is not
         mistaken for a dropped arm.

    Returns a dict of per-view evidence for the caller to log.
    """
    cur = conn.cursor()
    evidence: dict = {}
    problems: List[str] = []
    ...
    expected = {
        viewgen.ISSUES_VIEW: viewgen.ISSUES_VIEW_COLUMNS,
        viewgen.DEPENDENCIES_VIEW: viewgen.DEPENDENCY_VIEW_COLUMNS,
    }

    for view, want_cols in expected.items():
        qualified = f"{viewgen.quote_ident(db)}.{viewgen.quote_ident(view)}"
        try:
            cur.execute(f"SELECT * FROM {qualified} LIMIT 0")
            cur.fetchall()
            have = [c[0].lower() for c in cur.description]
        except Exception as exc:
            problems.append(
                f"{db}.{view} is not queryable ({type(exc).__name__}: {exc}) -- "
                "regenerate and apply adapters/board/create_view.sql"
            )
            evidence[view] = {"queryable": False}
            continue

        missing = [c for c in want_cols if c not in have]
        if missing:
            problems.append(
                f"{db}.{view} is missing column(s) {', '.join(missing)} -- the deployed "
                "view predates adapters/board/create_view.sql; regenerate and apply it"
            )

        try:
            cur.execute(f"SHOW CREATE VIEW {qualified}")
            definition = " ".join(str(x) for x in cur.fetchone())
        except Exception as exc:
            problems.append(f"{db}.{view}: SHOW CREATE VIEW failed ({type(exc).__name__}: {exc})")
            definition = ""

        absent = [p.db for p in viewgen.ROSTER if not viewgen.db_referenced_in(definition, p.db)]
        if definition and absent:
            problems.append(
                f"{db}.{view} omits roster DB(s) {', '.join(absent)} -- an arm was dropped; "
                "those projects would silently vanish from the total board"
            )
        ...
        evidence[view] = {
            ...
            "omitted_roster_dbs": absent,
        }

    if problems:
        raise BoardSourceError(
            "federated board views failed preflight:\n  - " + "\n  - ".join(problems)
        )
    return evidence
...
def fetch_open_blocked_by(conn) -> Dict[str, List[str]]:
    ...
    `d.depends_on_kind = 'issue'` is edge-identity preservation, not a filter
    on volume: bd >= 1.2 dependencies can target an issue, a wisp, or an
    external ref, and `all_dependencies.depends_on_id` COALESCEs all three so
    no edge is dropped from the union. Without the kind filter, a wisp or
    external id that happened to collide with an issue id would join to
    `all_issues` and fabricate a blocker out of nothing. Status semantics are
    unchanged: still `type = 'blocks'` and blocker not closed.
    """
    cur = conn.cursor()
    cur.execute(
        "SELECT d.issue_id, d.depends_on_id FROM all_dependencies d "
        "JOIN all_issues blocker ON blocker.id = d.depends_on_id "
        "WHERE d.type = 'blocks' AND d.depends_on_kind = 'issue' "
...
def run(
    *,
    ...
    try:
        # Fail closed BEFORE building anything: a degraded federated view
        # produces a board that looks healthy and is wrong (see
        # preflight_board_views).
        preflight_board_views(conn, dolt_db)
...
def main(argv: Optional[List[str]] = None) -> int:
    # argv is a parameter (not read straight from sys.argv) so the fail-closed
    # path is testable without a subprocess.
    ...
    except BoardSourceError as exc:
        # Exit non-zero WITHOUT writing board.json/report.md: a stale board on
        # disk is strictly better than a fresh one that under-reports blockers,
        # because consumers can see the stale generated_at and this run's
        # failure is visible in the timer's unit status.
        sys.stderr.write(f"[board] ABORTED -- {exc}\n")
