...
fn msys_tty_on(fd: DWORD) -> bool {
    ...
    // Mirrors winapi::um::fileapi::FILE_NAME_INFO with a fixed-length FileName so
    // we can stack-allocate it and avoid the unaligned-read flagged by
    // RUSTSEC-2021-0145 (the original code cast a Vec<u8> to *const FILE_NAME_INFO).
    #[repr(C)]
    ...
    let handle = unsafe {
        // Safety: GetStdHandle has no invariants; an invalid fd just yields
        // INVALID_HANDLE_VALUE which GetFileInformationByHandleEx will reject.
        GetStdHandle(fd)
    };
    let res = unsafe {
        // Safety: handle is whatever GetStdHandle returned; buffer has fixed
        // size matching the size we pass.
